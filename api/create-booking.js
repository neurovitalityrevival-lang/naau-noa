...
async function sendCAPI({ name, email, phone, sourceUrl }) {
  ...
  const pd = JSON.stringify({ data: [{ event_name: 'Schedule', event_time: Math.floor(Date.now()/1000), event_id: `sch_${Date.now()}`, action_source: 'website', event_source_url: sourceUrl || 'https://naau-noa.vercel.app/booking.html', user_data: ud, custom_data: { content_name: '無料オンライン個別相談予約' } }] });
  return new Promise((resolve, reject) => {
